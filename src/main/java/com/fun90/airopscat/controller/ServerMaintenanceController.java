...
import com.fun90.airopscat.model.dto.maintenance.ServerMaintenanceExecuteRequest;
...
import com.fun90.airopscat.service.maintenance.ServerMaintenanceService;
...
@ApplicationScoped
@Path("/api/admin/server-maintenance")
@Produces(MediaType.APPLICATION_JSON)
@Consumes(MediaType.APPLICATION_JSON)
public class ServerMaintenanceController {

    @Inject
    ServerMaintenanceService serverMaintenanceService;
    ...
    public Response getScripts() {
        List<MaintenanceScriptDto> scripts = serverMaintenanceService.listScripts();
    ...
    public Response previewScript(@jakarta.ws.rs.PathParam("scriptName") String scriptName) {
        ...
        try {
            String content = serverMaintenanceService.getScriptContent(scriptName);
            Map<String, Object> response = new HashMap<>();
    ...
    @POST
    @Path("/execute")
    public Response execute(ServerMaintenanceExecuteRequest request) {
        ...
        try {
            ServerMaintenanceStepResultDto result = serverMaintenanceService.executeScript(request.getServerId(), request.getScriptName());
            ...
            return Response.status(Response.Status.INTERNAL_SERVER_ERROR)
                    .entity(Map.of("message", "执行运维步骤失败: " + e.getMessage()))
                    .build();
        }
    }
}
