...
const nodeOnlineAccountStatsApp = {
    nodeId: null,
    nodeIdError: false,
    selectedDays: '30',
    loading: false,
    ...
    mounted() {
        ...
        if (!nodeId || Number.isNaN(Number(nodeId))) {
            this.nodeIdError = true;
            return;
        }
    ...
    async refreshData() {
        ...
        try {
            ...
            console.error(error);
            ToastUtils.show('Error', error.message || '加载节点在线趋势失败', 'danger');
        } finally {
            this.loading = false;
        }
    },
    ...
    async fetchSummary() {
        const response = await fetch(`/api/admin/node-online-account-stats/${this.nodeId}/summary?days=${this.selectedDays}`);
        if (!response.ok) {
            throw new Error(response.status === 404 ? '节点不存在' : '加载节点在线趋势摘要失败');
        }
        this.summary = await response.json();
    },

    async fetchCharts() {
        const response = await fetch(`/api/admin/node-online-account-stats/${this.nodeId}/charts?days=${this.selectedDays}`);
        if (!response.ok) {
            throw new Error(response.status === 404 ? '节点不存在' : '加载节点在线趋势图表失败');
        }
    ...
    renderChart() {
        ...
        const uniqueData = points.map(point => Number(point.uniqueOnlineAccountCount || 0));
        const peakData = points.map(point => Number(point.peakOnlineAccountCount || 0));
        const latestData = points.map(point => Number(point.latestOnlineAccountCount || 0));

        this.renderLineChart('#node-online-account-chart', categories, [
            { name: '每日活跃账户', data: uniqueData },
            { name: '峰值在线账户', data: peakData },
            { name: '最近采样在线', data: latestData }
        ]);
    },
    ...
    renderLineChart(selector, categories, series) {
        ...
        const options = {
            ...
            tooltip: {
                ...
                y: {
                    formatter: value => `${Number(value || 0).toFixed(0)} 个账户`
                }
            },
...
PetiteVue.createApp(nodeOnlineAccountStatsApp).mount('#app');
