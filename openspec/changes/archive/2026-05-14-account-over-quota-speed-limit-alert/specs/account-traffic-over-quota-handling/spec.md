...
系统 SHALL 在账户流量统计任务检测到账户流量超额或超额状态恢复后，请求现有限速配置刷新流程，使远端配置尽快包含重新计算后的有效速度。

#### Scenario: 账户进入流量超额状态
- **WHEN** 系统检测到账户从未超额变为流量超额
- **THEN** 系统 SHALL 触发限速同步（`triggerRateLimitSync` / `RateLimitService.syncAll()`）
