...
- [x] 6.2 手动访问 `/api/open/docs-info/*` 与 `/subscribe/*` 的示例路径，确认日志写入且包含客户端 IP
- [x] 6.3 手动访问非配置路径，确认不会写入请求日志
- [x] 6.4 手动验证日志列表按时间段、请求路径、客户端 IP 搜索与分页均正常
- [x] 6.5 手动验证 apexcharts 图表正常渲染，且按请求路径统计图存在
- [x] 6.6 手动执行或触发清理任务，确认 30 天以前日志被删除且近 30 天日志保留
