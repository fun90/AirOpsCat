...
限速链路已经存在：`Account.speed` 表示账号自身限速（`KB/s`，`0` 或 `null` 表示不限速），`RateLimitService` 会查询有效且 `speed > 0` 的账户，并将 `accountNo` 和 `speed` 写入服务器本地限速配置。新方案不能再把超额限速直接写入 `Account.speed`，否则会覆盖账户本身配置；应在生成部署客户端数据或限速同步数据时，根据账户是否超额计算本次下发的有效速度。
...
- 部署客户端数据生成时根据是否超额计算有效速度限制，不能直接把 `Account.speed` 当作最终速度。
...
**选择理由：** `Account.speed` 是管理员配置的账户自身限速，不应被流量超额处置覆盖。超额限速是由当前周期流量状态推导出来的运行时速度，应在生成部署客户端数据和限速同步数据时动态计算。这样既能保留管理员原始配置，也能在配额调整或周期切换后自然恢复到账号自身限速。

### 决策 2：去除部署阶段流量过滤

`DeploymentDataLoader#buildNodeClientsMap` SHALL 移除 `isWithinBandwidth(account, usedBytes)` 过滤，不再因为流量超额剔除账户。节点客户端列表只根据标签授权、账户有效期、账户禁用状态等条件决定是否包含账户。流量用量查询仍可用于计算有效速度，但不得用于过滤客户端。

### 决策 3：统计累加后触发超额处置

`AccountTrafficStatsService.saveOrUpdateTrafficStats()` 在新建或更新当前周期记录后调用超额处置服务。处置服务读取当前周期总用量、有效配额（优先 `bandwidthQuota`，兜底 `Account.bandwidth`），当总用量大于等于配额字节数时执行：
...
3. 按最小通知间隔发送 Bark 告警；
4. 触发或请求节点部署/限速配置刷新，使远端尽快拿到重新计算后的有效速度。

### 决策 4：部署客户端速度按超额状态计算

生成 `NodeClient` 时，系统 SHALL 为每个账户计算有效速度：
...
若后续通过修改配额、重置周期或人工清流量使账户不再超额，系统只将 `AlertState` 更新为 `RECOVERED`。由于超额限速不写入 `Account.speed`，下一次部署客户端数据生成时会自动回到 `Account.speed` 定义的账户自身限速。
...
- **[风险] 动态速度计算与远端限速同步来源不一致** → 将有效速度计算封装为可复用方法，部署数据生成与限速同步共用同一逻辑。
...
- **[权衡] 部署阶段需要查询流量状态** → 相比直接读账户字段多一次状态计算，但换来不污染账户配置和周期切换后自动恢复。
